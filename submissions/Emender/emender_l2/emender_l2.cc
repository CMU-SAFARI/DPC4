#include "emender_l2.h"

#include <cassert>
#include <iostream>

#include "../emender_l1d/emender_l1d.h"
#include "../pythia/learning_engine_featurewise.h"
#include "../pythia/pythia_helper.h"
#include "../pythia/pythia_params.h"
#include "cache.h"
#include "champsim.h"
#include "dpc_api.h"
#include "modules.h"

// modified pythia that sends its miss rate to l3
struct custom_pythia : public champsim::modules::prefetcher {
private:
  ...
  PythiaStats stats;
  bool throttle;
  uint64_t last_useless;
  uint64_t last_cycle;
  uint64_t cur_useless;
...
void custom_pythia::prefetcher_initialize()
{
  init_knobs();
...
uint32_t custom_pythia::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                                 uint32_t metadata_in)
{
  ...
  for (uint32_t addr_index = 0; addr_index < pref_addr.size(); ++addr_index) {
    ...
    uint32_t metadata = (2 << 30) | (intern_->cpu << 28) | cur_useless;
    if (cur_useless >= (1 << 28)) {
      // overflow may occur when counters are reset
      metadata = 0;
    }
    intern_->prefetch_line(pf_addr, true, metadata);
  }

  return 0;
}

uint32_t custom_pythia::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr,
                                              uint32_t metadata_in)
{
  if ((metadata_in >> 30) == 3) {
    int throttle = metadata_in & 1;
    if (throttle != this->throttle) {
      printf("L2 CPU %d throttle %d -> %d\n", intern_->cpu, this->throttle, throttle);
    }
...
void custom_pythia::prefetcher_cycle_operate()
{
  auto current_cycle = intern_->current_time.time_since_epoch() / intern_->clock_period;
  if (current_cycle - last_cycle > 1024 * 128) {
    last_cycle = current_cycle;
    uint64_t new_miss = intern_->sim_stats.pf_useless - last_useless;
    // running average
    cur_useless = cur_useless * 0.3 + new_miss * 0.7;
    last_useless = intern_->sim_stats.pf_useless;
  }
}
...
void custom_pythia::update_global_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address) { /* @rbera TODO: implement */ }

Scooby_STEntry* custom_pythia::update_local_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address)
{
  stats.st.lookup++;
...
//----------------------------------------------------//
uint32_t custom_pythia::predict(uint64_t base_address, uint64_t page, uint32_t offset, State* state, std::vector<uint64_t>& pref_addr)
{
  ...
  if (PYTHIA::enable_dyn_degree && !(throttle && ENABLE_L3_THROTTLE)) {
...
//----------------------------------------------------//
bool custom_pythia::track(uint64_t address, State* state, uint32_t action_index, Scooby_PTEntry** tracker)
{
...
//----------------------------------------------------//
uint32_t custom_pythia::get_dyn_pref_degree(float max_to_avg_q_ratio, uint64_t page, int32_t action)
{
...
//----------------------------------------------------//
void custom_pythia::gen_multi_degree_pref(uint64_t page, uint32_t offset, int32_t action, uint32_t pref_degree, std::vector<uint64_t>& pref_addr)
{
...
//----------------------------------------------------//
void custom_pythia::reward(uint64_t address)
{
...
//----------------------------------------------------//
void custom_pythia::reward(Scooby_PTEntry* ptentry)
{
...
//----------------------------------------------------//
void custom_pythia::assign_reward(Scooby_PTEntry* ptentry, RewardType type)
{
...
//----------------------------------------------------//
int32_t custom_pythia::compute_reward(Scooby_PTEntry* ptentry, RewardType type)
{
...
//----------------------------------------------------//
void custom_pythia::train(Scooby_PTEntry* curr_evicted, Scooby_PTEntry* last_evicted)
{
...
//----------------------------------------------------//
void custom_pythia::register_fill(uint64_t address)
{
...
std::vector<Scooby_PTEntry*> custom_pythia::search_pt(uint64_t address, bool search_all)
{
...
int32_t custom_pythia::getAction(uint32_t action_index)
{
  assert(action_index < Actions.size());
  return Actions[action_index];
}

void custom_pythia::track_in_st(uint64_t page, uint32_t pred_offset, int32_t pref_offset)
{
  auto st_index = find_if(signature_table.begin(), signature_table.end(), [page](Scooby_STEntry* stentry) { return stentry->page == page; });
  if (st_index != signature_table.end()) {
...
bool custom_pythia::is_high_bw(uint8_t bw_level)
{
  if (ENABLE_L3_THROTTLE) {
    return (bw_level >= PYTHIA::high_bw_thresh || throttle) ? true : false;
  } else {
    return bw_level >= PYTHIA::high_bw_thresh ? true : false;
  }
}

void emender_l2::prefetcher_initialize()
{
  inner = new custom_pythia(intern_);
  inner->prefetcher_initialize();
}

uint32_t emender_l2::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                              uint32_t metadata_in)
{
  return inner->prefetcher_cache_operate(addr, ip, cache_hit, useful_prefetch, type, metadata_in);
}

uint32_t emender_l2::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  return inner->prefetcher_cache_fill(addr, set, way, prefetch, evicted_addr, metadata_in);
}

void emender_l2::prefetcher_cycle_operate() { inner->prefetcher_cycle_operate(); }

void emender_l2::prefetcher_final_stats() { inner->prefetcher_final_stats(); }